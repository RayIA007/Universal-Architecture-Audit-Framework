"""
PATCH-RUNTIME-PIPELINE-COMMIT-0004B-REVISED

Integrate PipelineFailureHandler into RuntimePipeline.

This revised patch fixes the import-anchor detection bug in the previous
version. It is idempotent and provides backup, AST validation, compilation
...
def validate_ast(source: str) -> None:
    ...
    handler_import_found = any(
        ...
        and any(alias.name == "PipelineFailureHandler" for alias in node.names)
        for node in ast.walk(tree)
    )
    ...
    if not handler_import_found:
        raise RuntimeError("PipelineFailureHandler import was not found.")
    ...
    if not decision_condition_found:
        raise RuntimeError("decision.should_stop condition was not found.")


def has_handler_import(source: str) -> bool:
    tree = ast.parse(source, filename=str(TARGET))
...
def add_handler_import(source: str) -> str:
    if has_handler_import(source):
        return source
    ...
    matches = [
        ...
        if line.strip() == EXECUTOR_IMPORT
    ]

    if len(matches) != 1:
        raise RuntimeError(
            "Expected exactly one PipelineExecutor import line; "
            f"found {len(matches)}."
        )
    ...
    import_lines = [
        f"from uaaf_core.runtime.pipeline_failure_handler import ({newline}",
        f"    PipelineFailureHandler,{newline}",
        f"){newline}",
    ]

    lines[index + 1:index + 1] = import_lines
    return "".join(lines)


def transform(source: str) -> str:
    transformed = add_handler_import(source)
...
def main() -> int:
    ...
    try:
        ...
        py_compile.compile(str(TARGET), doraise=True)
        validate_ast(TARGET.read_text(encoding="utf-8"))
